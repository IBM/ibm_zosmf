                    ...
                    stage('Build') {
                        steps {
                            ...
                            script {
                                ...
                                dir("${remoteWorkspace}") {
                                        if (fileExists('ibm-ibm_zosmf-1.2.1.tar.gz')) {
                                                echo "ibm-ibm_zosmf-1.2.1.tar.gz existed on ${SSH_PORT}"
                                                sh 'rm ibm-ibm_zosmf-1.2.1.tar.gz'
                                                sh 'ansible-galaxy collection build --force'
                                        } else {
                                                sh 'ansible-galaxy collection build --force'
                                        }
                                        sh "pwd"
                                        sh 'ansible-galaxy collection install ibm-ibm_zosmf-1.2.1.tar.gz --force'
                                }
                            }
                        }
                    }
