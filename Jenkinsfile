		...
		stage('Build') {
			...
			steps {
				...
				script {
					...
					dir("${remoteWorkspace}") {
							if (fileExists('ibm-ibm_zosmf-1.0.1.tar.gz')) {
									echo "ibm-ibm_zosmf-1.0.1.tar.gz existed"
									sh 'rm ibm-ibm_zosmf-1.0.1.tar.gz'
									sh '/usr/bin/ansible-galaxy collection build'
							} else {
									sh '/usr/bin/ansible-galaxy collection build'
							}
							sh "pwd"
							sh '/usr/bin/ansible-galaxy collection install ibm-ibm_zosmf-1.0.1.tar.gz'
					}
				}
            }
		}
